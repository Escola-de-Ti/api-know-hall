...
public interface PostRepository extends JpaRepository<Post, BigInteger> {
  ...
  @Query(value = """
    ...
    paginated_posts AS (
        ...
        ORDER BY
            CASE WHEN :orderBy = 'RELEVANCE' THEN relevance_score END DESC,
            CASE WHEN :orderBy = 'UPVOTES_DESC' THEN COALESCE(total_up_votes, 0) END DESC,
    ...
    LEFT JOIN votos v
            ...
            AND v.tipo = 'UP_VOTE'
    ORDER BY
        CASE WHEN :orderBy = 'RELEVANCE' THEN pp.relevance_score END DESC,
        CASE WHEN :orderBy = 'UPVOTES_DESC' THEN COALESCE(pp.total_up_votes, 0) END DESC,
        CASE WHEN :orderBy = 'UPVOTES_ASC' THEN COALESCE(pp.total_up_votes, 0) END ASC,
        CASE WHEN :orderBy = 'DATE_DESC' THEN pp.data_criacao END DESC,
        CASE WHEN :orderBy = 'DATE_ASC' THEN pp.data_criacao END ASC,
        pp.id DESC
    """, nativeQuery = true)
